...
} if (xPos > canvasWidth - imageWidth) {
...
const renderCards = () => {
  ...
  for (let row = 0; row < rowsData.length; row++) {
    ...
    for (let card = 0; card < rowsData[row].images.length; card++) {
      ...
      } else if (shouldRender(xPos, xPos + imageWidth, yPos, yPos + imageHeight)) {
...
const updateYPosition = (easing) => {
  if (highlightCurrentPositionY > selectedCardTargetPositionY) {
    // down
    translateY =
      targetTranslateY - singleMoveDistanceY + (singleMoveDistanceY + unfinishedMovementY) * easing;
  }
  if (highlightCurrentPositionY < selectedCardTargetPositionY) {
    // up
    translateY =
