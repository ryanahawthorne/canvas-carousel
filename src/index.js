            ...
            switch (keyCode) {
                case 37: // left
                    if (selectedRowObject.highlightedCard > 0) { // boundary check
                        ...
                        selectedRowObject.targetTranslateX = (selectedRowObject.highlightedCard * singleMoveDistanceX);
                        draw();
                    }
                    break;
            ...
            loadImageArray().then(function (data) {
                data.results.forEach((image, index) => {
                    ...
                    loadImage(fullImageUrl).then(image => { // TODO callback to render image after loading is very basic and should instead draw the image based on it's real location in case things load in late
                        ...
                        const xPos = applyTranslate(cardOriginalPositionX + selectedCardTargetPositionX, translateX);
                        const yPos = applyTranslate(cardOriginalPositionY + selectedCardTargetPositionY, translateY);
            ...
            for (let row = 0; row < rowsData.length; row++) {
                ...
                if (rowObject.animationStartTime) {
                    ...
                    if (rowObject.animationDirection === 'right') { // if right
                        rowObject.translateX = (rowObject.targetTranslateX - singleMoveDistanceX) + ((singleMoveDistanceX + unfinishedMovementX) * easingPosition);
                ...
                for (let card = 0; card < rowsData[row].images.length; card++) {
                    ...
                    const xPos = applyTranslate(cardOriginalPositionX + unfinishedMovementX + selectedCardTargetPositionX, rowObject.translateX); // translateX affects only current row
                    const yPos = applyTranslate(cardOriginalPositionY + selectedCardTargetPositionY + unfinishedMovementY, translateY); // translateY affects all rows
                    ...
                    if (row === selectedRow && card == highlightedCard) {
                        ...
                        drawSelectedCardLast = () => { // set this as a callback so the highlighted card is drawn last to prevent it clipping behind other cards
                            ...
                            ctx.drawImage(image, xPos - xOffset, yPos - yOffset, imageWidth * imageScaling, imageHeight * imageScaling);
                            ctx.globalAlpha = 1;
        ...
        const drawFocusHighlight = (highlightedXIn, highlightedYIn, imageScaling) => {
            ...
            ctx.strokeStyle = "#ccc";
            ctx.beginPath();
            ctx.rect(highlightedXIn - xOffset, highlightedYIn - yOffset, imageWidth * imageScaling, imageHeight * imageScaling);
            ctx.stroke();
        }

        loadImage(`https://image.tmdb.org/t/p/w${imageWidth}/pSh8MyYu5CmfyWEHzv8FEARH2zq.jpg`).then(img => {
        ...
        const updateYPosition = (easing) => {
            if (highlightCurrentPositionY > selectedCardTargetPositionY) { // down
                translateY = (targetTranslateY - singleMoveDistanceY) + ((singleMoveDistanceY + unfinishedMovementY) * easing);
            }
            if (highlightCurrentPositionY < selectedCardTargetPositionY) { // up
                translateY = (targetTranslateY + singleMoveDistanceY) - ((singleMoveDistanceY - unfinishedMovementY) * easing);
            }
        }
        ...
        const draw = () => {
            ...
            highlightCurrentPositionY = Math.round(applyTranslate((selectedRow * singleMoveDistanceY) + selectedCardTargetPositionY + unfinishedMovementY, translateY));
